...
This feature can be disabled by setting the option \texttt{UseSpecialAlgorithms} (default value \texttt{true}) to \texttt{false}; in Section~\ref{ss: UseSpecialAlgorithms} we give discuss situations where the user might want to do that.
%Consider the following example, where $f$ is a diagonal polynomial.
\todo{Explain why the user might want to disable this.}
\daniel{Why would one want to do this?}
\pedro{In Section~\ref{ss: UseSpecialAlgorithms} we give a couple of examples, so I'm pointing the reader to that section.}
...
%may produce a different result than the other two values.
...
% The functions \texttt{testIdeal} and \texttt{FPureModule} therein are used to compute the test ideals of $f$ with respect to the parameter $t$, and that with respect to $t - \varepsilon$, for $0<\varepsilon\ll 1$. For instance, as noted in the introduction, when $R$ is a polynomial ring, $t$ is the $F$-pure threshold of $f$ if and only if $\tau(R, f^t)$ is contained in the homogeneous maximal ideal, but
...
\pedro[inline]{I modified the code, to produce the output you suggested.}
\daniel[inline]{OK.  I will just manually make this change in the verbatim environment below then, in case we decide to keep it here.}
