...
%
Given an reduced ring $R$ of finite type over a perfect field $k$, the Frobenius map $R \hookrightarrow R^{1/p^e}$ is dual to $T : \omega_{R^{1/p^e}} \to \omega_R$.  As in \autoref{subsec.DualToFrobeniusOnQuotientRings}, we can represent the canonical module $\omega_R \subseteq R$ as an ideal, we can write $R = S/I$, and so we can find an element $u \in S^{1/p^e}$ representing the map $T:\omega_{R^{1/p^e}} \to \omega_R$; see \autoref{subsec.DualToFrobeniusOnQuotientRings} or \cite{KatzmanParameterTestIdealOfCMRings}.
...
The method \texttt{compatibleIdeals} produces the finite set of $\phi$-compatible prime ideals in the second claim of Theorem \ref{Theorem: finitely many compatible primes}.
...
In \cite{KatzmanZhangAlgorithm}, the algorithms behind the methods in Section \ref{Section: compatible ideals} were extended
