...
% \begin{center}
% {\large \textbf{The \emph{FrobeniusThresholds} package for \emph{Macaulay2}}}
% \end{center}
...
The functionality implemented in the \emph{FrobeniusThresholds} is not restricted to nonsingular ambient rings (see Section \ref{sec.IsFPT}).
However, for most of this article, we work in a polynomial ring over a finite field of  characteristic $p>0$, and we remain in this case throughout the introduction for the sake of clarity.  
Unless otherwise stated, all polynomials and ideals under consideration are regarded as elements in such a ring.  The ideal of this ring generated by the ring's variables is denoted $\idealm$.

Let us roughly outline the way in which many natural numerical invariants in prime characteristic commutative algebra are constructed:  For every natural number $e$, associate to some fixed set of data|often, a collection of polynomials and ideals|an integer describing something of relevance that depends on $e$ (e.g., the dimension of some interesting vector space constructed in terms of your initial data).   Finally, normalize this integer by dividing by some power of $p^e$, and take the limit as $e$ approaches infinity.
...
%Erin Bela, Juliette Bruce, Sara Malec, Maral Mostafazadehfard, Marcus Robinson, Matthew Mastroeni
...
The function {\tt nu} works by searching through the list of potential integers $n$ and checking containments of the $n^\text{the}$ power of $I$ in a specified Frobenius power of $J$.  
The user can specify how this search is approached through the  option {\tt Search}.  Namely, a binary search is used when {\tt Search} is set to {\tt Binary} (and this is the default value), and a linear search is run when it is set to {\tt Linear}.

The option {\tt ReturnList} for {\tt nu} (whose default value is {\tt false}) can be set to {\tt true} so that instead of outputting $\nu_I^J(p^e)$, but a list contaning $\nu_I^J(p^i)$, for $i=0,\ldots,e$.
Alternatively, the option {\tt Verbose} can be set to  to request that the values $\nu_I^J(p^i)$ ($i=0,\ldots,e$) be printed as they are computed, to monitor the progress of the computation.
...
In line with the philosophy described in the introduction, after appropriately normalizing and taking the limit of these values as $e$ approaches infinity, these new values limit to certain invariants of interest called \emph{critical exponents} (see \emph{loc.~cit.}).
