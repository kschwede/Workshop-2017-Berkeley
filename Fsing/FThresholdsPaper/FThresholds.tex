...
Notice that $\nu_I^\idealm(p^e)$ can be interpreted as the maximum integer $n>0$ for which $(I^n)^{[1/p^e]} \not\subseteq \idealm$.
When computing its value globally, it is the maximum $n>0$ for which $(I^n)^{[1/p^e]} \neq R$, and when 
the value of {\tt IsLocal} is set to {\tt false} (from its default {\tt true}), then {\tt nu} ignores the option {\tt ContainmentTest} and instead calculates determines this value $n$.
