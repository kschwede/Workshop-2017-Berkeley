...
In the literature, the $F$-pure threshold $\ft^{\idealm}(f)$ is often denoted $\fpt(f)$, for obvious reasons.  However, in this note, we adopt the former notation to avoid any possible confusion with the \emph{Macaulay2} function \texttt{fpt} described in Section \ref{sec.FPT}.  Indeed, the command \texttt{fpt(f)} need not always output the number $\ft^{\idealm}(f) = \fpt(f)$, though in the case that it cannot, it will provide a reasonably good estimate for this number.
...
We highlight some degenerate cases:  If $f$ is the zero polynomial, then \texttt{nu(e,f,J)} outputs \texttt{0}, and if $f$ is \emph{not} contained in the radical of $J$, then the output is instead \texttt{infinity}.  These degenerate cases extend analogously to the command \texttt{nu(e,I,J)}.
...
This fact is used to speed up computations for certain polynomials whose $F$-pure thresholds can be computed quickly via specialized algorithms, namely diagonal polynomials, binomials, and forms in two variables (see Section \ref{sec.FPT}).  This feature can be disabled by setting the option \texttt{UseSpecialAlgorithms} (default value \texttt{true}) to \texttt{false}.  Consider the following example, where $f$ is a diagonal polynomial.
...
We now describe the options available for the function \texttt{nu}.  We focus primarily on the case where the second input is an ideal $I$, since the options work analogously when it is instead a polynomial $f$.
...
The user can specify how this search is approached through the  option \texttt{Search}.  Namely, a binary search is used when \texttt{Search} is set to \texttt{Binary} (and this is the option's default value), and a linear search is run when it is set to \texttt{Linear}.
...
i18 : f = x^2*y^4 + y^2*z^7 + z^2*x^8;

i19 : time nu(5, f) -- uses binary search (default)
     -- used 0.350179 seconds

o19 = 1124

i20 : time nu(5, f, Search => Linear)
     -- used 0.485823 seconds
...
\section{\texttt{isFPT}, \texttt{compareFPT} and \texttt{isFJumpingExponent}}
\label{sec.IsFPT}
...
\section{The \texttt{fpt} function}
\label{sec.FPT}
...
First, a number in the interval is selected, according to criteria specified by the value of the option \texttt{GuessStrategy}; we refer the reader for the documentation of this option for more details.
Then the function \texttt{compareFPT} is used to test that number. If that ``guess'' is correct, its value is returned; otherwise, the data returned by \texttt{compareFPT} is used to narrow down the interval, and this process is repeated as many times as specified by \texttt{Attempts}.
...
\setstretch{.67}
\begin{MyVerbatim}
i30 : f = x^7*y^5*(x + y)^5*(x^2 + y^3)^4;

i31 : fpt(f, DepthOfSearch => 3, FinalAttempt => true, Verbose => true)
...
ν has been computed: ν = ν(3,f) = 9 ...

fpt lies in the interval [ν/(p^e-1),(ν+1)/p^e] = [9/124,2/25] ...
...
First F-signature computed: s(f,(ν-1)/p^e) = 456/15625 ...

Second F-signature computed: s(f,ν/p^e) = 64/15625 ...
...
i40 : f = (y - (x - 1)^2)*y^2; --SNC at the origin, but not globally
...
i43 : g = x*y^2*(x + 1)^3*(y - 1)^4; --SNC everywhere

i44 : isSimpleNormalCrossing(g)

o44 = true

i45 : isSimpleNormalCrossing(g, IsLocal => false)
