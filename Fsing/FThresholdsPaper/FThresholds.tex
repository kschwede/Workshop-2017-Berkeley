...
If $I = (f)$ is principal, then we call $\ft^{J}(f) := \ft^J(I)$  the \emph{$F$-threshold of $f$ with respect to $J$}, 
and $\ft^{\idealm}(I)$ is the \emph{$F$-pure threshold of $I$}. 
Like $F$-pure thresholds, the $F$-thresholds of $I$ can also be defined in terms of its test ideals.
...
Finally, suppose that {\tt ContainmentTest} is set to {\tt FrobeniusPower}.  Instead of producing the invariant $\nu_I^J(p^e)$, in this case the {\tt nu} function instead outputs the maximal integer $n$ such that the $n^{\textrm{th}}$ (generalized) Frobenius power of $I$ is not contained in the $p^e$-th Frobenius power of $J$ \cite{hernandez+etal.frobenius_powers}.  Note that this coincides with $\nu_I^J(p^e)$ when $I$ is replaced with a polynomial, or is a principal ideal.
After appropriately normalizing and taking the limit of these values as $e$ approaches infinity, these new values limit to certain \emph{critical exponents} (see \emph{loc.~cit.}).
