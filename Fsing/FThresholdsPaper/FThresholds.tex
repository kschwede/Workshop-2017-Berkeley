...
The \emph{FThresholds} package contains methods to 
...
The general method applied calls upon functionality from the \emph{TestIdeals} package (CITE).  The functions {\tt testIdeal} and {\tt FPureModule} therein are used
...
Note that when $R$ is a strongly $F$-regular Gorenstein ring, {\tt FPureModule(t, f)} precisely computes $\tau(R, f^{t-\varepsilon})$.  
