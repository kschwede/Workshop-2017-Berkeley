...
When $f$ is a monomial, say, $x_1^{a_n}\cdots x_n^{a_n}$, then its $F$-pure threshold is quickly computed as the minimum among the $1/a_i$.
When $f$ is a a diagonal polynomial, a binomial, a form in two variables, 
algorithms of Hern\'andez \cite{HernandezFInvariantsOfDiagonalHyp, HernandezFPureThresholdOfBinomial}, or Hern\'andez and Teixeira \cite{HernandezTeixeiraFThresholdFunctions},  are executed to compute the $F$-pure threshold (cf. \cite{ShibutaTakagiLCThresholds}).  
Consider examples of each of these cases below.
...
A number in the interval is selected, according to criteria specified by the option \textcolor{red}{{\tt GuessStrategy}}, and the function {\tt compareFPT} is used to test that number. If this ``guess'' is correct, its value is returned. Otherwise, the information returned by {\tt  compareFPT} is used to narrow down the interval, and this process is repeated as many times as specified by the value of {\tt Attempts}.
...
i20 : fpt(f, DepthOfSearch => 3, Attempts => 4) -- more attempts *always*
...
i21 : fpt(f, DepthOfSearch => 3, Attempts => 4, GuessStrategy => denominator) 
...
i67 : f = x^7*y^5*(x + y)^5*(x^2 + y^3)^4;

i68 : fpt(f, DepthOfSearch => 3, Attempts => 5)
...
i69 : fpt(f, DepthOfSearch => 3, Attempts => 5, Bounds => oo)
...
i5 : isSimpleNormalCrossing(f, IsLocal=>false)

o5 = false

i6 : g = (y-1)^2 + (x-7)^2 -- doesn't even pass through the origin
...
i7 : isSimpleNormalCrossing(g)

o7 = true

i8 : isSimpleNormalCrossing(g, IsLocal=>false)

o8 = false

i9 : h = x*y^2*(x+1)^3*(y-1)^4; --SNC everywhere

i10 : isSimpleNormalCrossing(h)

o10 = true

i11 : isSimpleNormalCrossing(h, IsLocal=>false)
...
If $f = x_1^{a_n}\cdots x_n^{a_n}$, then its $F$-pure threshold is simply $\min \{ 1/a_1, \ldots, 1/a_n \}$.  When $f$ is a diagonal polynomial, a binomial, or a form in two variables, 
algorithms of Hern\'andez \cite{HernandezFInvariantsOfDiagonalHyp, HernandezFPureThresholdOfBinomial}, or Hern\'andez and Teixeira \cite{HernandezTeixeiraFThresholdFunctions},  are executed to compute the $F$-pure threshold.  We note that algorithms for computing $F$-pure thresholds of binomials in certain special characteristics appear in \cite{ShibutaTakagiLCThresholds}.
...
\subsection*{Searching for the $F$-pure threshold when no special algorithm applies}
...
\end{document}
>>>>>>> c768df2d40aceec6dd5553f80fb91ea3776825d8
