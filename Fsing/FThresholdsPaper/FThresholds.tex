...
\begin{abstract}
   ...
   This package is designed to compute and estimate $F$-pure thresholds, more general $F$-thresholds, and related numerical invariants arising in the study of singularities in prime characteristic commutative algebra.
\end{abstract}
...
\subsection*{Acknowledgements.}  First and foremost, the authors enthusiastically thank all authors 
and contributors to the \emph{FThresholds} package: (INSERT NAMES HERE). %Erin Bela, Juliette Bruce, Zhibek Kadyrsizova, Sara Malec, Maral Mostafazadehfard, Marcus Robinson, Dan Smolkin, and Robert Walker.
...
The function {\tt isFPT} determines whether a rational number is the $F$-pure threshold of a specified polynomial. 
Given a rational number $t$ and an element $f$ of a $\mathbb{Q}$-Gorenstein ring of characteristic $p>0$, the command {\tt isFPT(t,f)} outputs {\tt true} if $t$ is the $F$-pure threshold of $f$, and {\tt false} otherwise. 
The function {\tt compareFPT} provides further information about the relationship between $t$ and the $F$-pure threshold of $f$; namely, 
{\tt compareFPT(t, f)} returns {\tt-1} when $t < \fpt(f)$, $0$ when $t = \fpt(f)$ and $1$ when $t > \fpt(f)$.
Consider the following example, that illustrates this functionality. 
...
In general, this is done by computing the \emph{test ideal} associated to the pair $(R, f^t)$, and that associated when $t$ is replaced with a value less than $t$ that is very close to $t$, that associated to the pair $(R, f^{t-\epsilon}s)$. 
These values are computed using the functions 
{\tt testIdeal} and {\tt FPureModule} from the \emph{TestIdeals} package
(CITE THE TEST IDEALS PACKAGE PAPER).
In fact, $t$ is the $F$-pure threshold of $f$ if and only if $\tau(R, f^{t-\epsilon})$ is $R$, but  $\tau(R, f^t)$ is a proper ideal.  
Note that when $R$ is a strongly $F$-regular Gorenstein ring, {\tt FPureModule(t, f)} precisely computes $\tau(R, f^{t-\epsilon})$.  \textcolor{red}{These commands work more generally, but they \emph{only} currently work when $R$ is $\bQ$-Gorenstein, with index not divisible by $p$.}

The function {\tt compareFPT} provides further information about the relationship between $t$ and the $F$-pure threshold of $f$.  Namely, 
{\tt compareFPT(t, f)} returns {\tt-1} when $t < \fpt(f)$, $0$ when $t = \fpt(f)$ and $1$ when $t > \fpt(f)$.

Consider the following example.  Here, the $F$-pure threshold of $f$ equals to $1/2$; see \cite[Example 4.3]{MustataYoshidaTestIdealVsMultiplierIdeals}.
\medskip
{\small
\setstretch{.67}
\begin{MyVerbatim}
...
The function {\tt isFJumpingExponent} is implemented in a similar way, using the fact that a value $t$ is an $F$-jumping exponent if and only if the test ideals 
$\tau(R, f^t)$ and $\tau(R, f^{t-\epsilon})$ differ. 
Again, this command is implemented in rings that are $\bQ$-Gorenstein with index not divisible by $p$.
