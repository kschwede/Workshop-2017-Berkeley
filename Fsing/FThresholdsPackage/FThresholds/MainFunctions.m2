...
(
	...
	(
            ...
            computedHSLGInitial = HSLGModule( { a1rem/(pp^c1-1) }, { f }, CanonicalIdeal=>baseTau#0, GeneratorList=>{ h1 } );
            computedHSLG = frobeniusRoot( b1, { ceiling( (pp^b1 - 1)/(pp-1) ), a1quot }, { h1, sub( f, S1 ) }, sub( computedHSLGInitial#0, S1 ) );
        )
        else (
            computedHSLGInitial = HSLGModule( { a1/(pp^c1-1) }, { f }, CanonicalIdeal=>baseTau#0, GeneratorList=>{ h1 } );
...
compareFPTPoly(Number, RingElement) := o -> (t, f) -> (
    ...
    (
        a1quot := floor( (a1-1)/(pp^c1-1) );
        a1rem := a1 - (pp^c1-1)*a1quot;
        computedHSLGInitial = HSLGModule( { a1rem/(pp^c1-1) }, { f }, CanonicalIdeal=>ideal 1_S1, GeneratorList=>{ h1 } );
        computedHSLG = frobeniusRoot( b1, { ceiling( (pp^b1-1)/(pp-1) ), a1quot }, { h1, f }, computedHSLGInitial#0 );
    )
    else
    (
        computedHSLGInitial = HSLGModule( { a1/(pp^c1-1) }, { f }, CanonicalIdeal=>ideal 1_S1, GeneratorList=>{ h1 } );
	--the e is assumed to be 1 here since we are implicitly doing stuff
...
(
	...
	(
            ...
            computedHSLGInitial = HSLGModule({a1/(pp^c1 - 1)}, {f}, CanonicalIdeal=>baseTau#0, GeneratorList=>{h1}); --the e is assumed to be 1 here since we are implicitly doing stuff
            computedHSLG = frobeniusRoot(b1, ceiling( (pp^b1 - 1)/(pp-1) ), h1, sub(computedHSLGInitial#0, S1))
        )
    )
    else--there should be an algorithm that works here
...
(
    ...
    if (a1 > (pp^c1-1)) then(
        a1quot := floor( (a1-1)/(pp^c1 - 1));
        a1rem := a1 - (pp^c1-1)*a1quot;
        computedHSLGInitial = HSLGModule({a1rem/(pp^c1-1)}, {f}, CanonicalIdeal=>ideal(sub(1, S1)), GeneratorList=>{h1});
        computedHSLG = frobeniusRoot(b1, {ceiling( (pp^b1 - 1)/(pp-1) ), a1quot}, {h1, f}, computedHSLGInitial#0);
    )
    else (
        computedHSLGInitial = HSLGModule({a1/(pp^c1 - 1)}, {f}, CanonicalIdeal=>ideal(sub(1, S1)), GeneratorList=>{h1}); --the e is assumed to be 1 here since we are implicitly doing stuff
        computedHSLG = frobeniusRoot(b1, ceiling( (pp^b1 - 1)/(pp-1) ), h1, computedHSLGInitial#0);
    );
